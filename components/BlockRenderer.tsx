...
const BlockRenderer: React.FC<BlockRendererProps> = ({ block }) => {
  ...
  const highlightText = (text: string) => {
    ...
    // Split the text by the search query, ignoring case
      ...
      <>
        {parts.map((part, i) =>
          part.toLowerCase() === searchQuery.toLowerCase()
            ? <mark key={i} className="bg-yellow-300 text-gray-900 rounded-sm px-0.5 shadow-sm">{part}</mark>
            : part
        )}
      </>
    );
  ...
  switch (block.type) {
    case BlockType.TITLE_COVER:
      return <h1 className="text-4xl sm:text-5xl font-black text-blue-600 tracking-tight text-center mt-32 md:mt-40 mb-8 leading-tight drop-shadow-sm">{highlightText(block.content)}</h1>;

    case BlockType.VERSION:
      return <div className="text-base sm:text-lg text-gray-400 mt-2 text-center mb-32 md:mb-40 font-medium tracking-widest uppercase">{highlightText(block.content)}</div>;

    case BlockType.HEADING_1:
      return (
        <div className="mt-5 mb-3 group">
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 mb-1">{highlightText(block.content)}</h1>
          <div className="h-1.5 w-12 sm:w-16 bg-blue-500 rounded-full transition-all group-hover:w-20"></div>
        </div>
      );

    case BlockType.HEADING_2:
      return <h2 className="text-lg sm:text-xl text-blue-600 font-bold mt-4 mb-2 tracking-tight border-l-4 border-blue-600 pl-2 sm:pl-3">{highlightText(block.content)}</h2>;

    case BlockType.HEADING_3:
      return <h3 className="text-base sm:text-lg text-gray-800 font-bold mt-3 mb-1">{highlightText(block.content)}</h3>;

    case BlockType.DISCLAIMER_TITLE:
      return <div className="text-base sm:text-lg font-bold mb-2 mt-4 text-gray-950 border-b border-gray-100 pb-1">{highlightText(block.content)}</div>;
    ...
    case BlockType.NOTE:
      return (
        <div className="relative bg-blue-50/40 border-l-4 border-blue-500 p-2 sm:p-3 my-2 sm:my-3 rounded-r-lg sm:rounded-r-xl overflow-hidden group">
          ...
          <span className="text-[11px] sm:text-xs text-gray-800 leading-snug sm:leading-relaxed font-medium">{highlightText(block.content.replace(/^Nota:/, '').replace(/^Note:/, '').trim())}</span>
        </div>
      );
    ...
    case BlockType.TOC_ITEM:
      return (
        <div className="flex justify-between items-baseline mb-0.5 sm:mb-1 text-[12px] sm:text-[13px] group cursor-default">
          <span className="font-semibold text-gray-800 group-hover:text-blue-600 transition-colors">{highlightText(block.content)}</span>
    ...
    default: // PARAGRAPH
      return <p className="mb-2 sm:mb-3 text-[12px] sm:text-[13px] leading-snug text-gray-700 text-justify">{highlightText(block.content)}</p>;
  }
};
